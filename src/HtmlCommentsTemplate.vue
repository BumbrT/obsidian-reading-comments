	...
	<NConfigProvider :theme="theme">
		<NSpace vertical>
			<NSpace v-if="plugin.settings.showDefaultTagEditor">
				<NButton size="small" circle @click="saveDefaultTagValue">
					<template #icon>
						<Icon>
							<SaveSharp :style="iconColor"/>
						</Icon>
					</template>
				</NButton>
		...
		<NTree block-line expand-on-click :default-expand-all="plugin.settings.autoExpand" :pattern="searchPattern"
			   ...
			   :on-update:selected-keys="jumpToCommentOrExpandTag"
...
import {SettingsBackupRestoreSharp, SaveSharp} from '@vicons/material';
...
function saveDefaultTagValue() {
	plugin.settings.defaultTagEditorValue = defaultTag.value
	plugin.saveSettings()
}
